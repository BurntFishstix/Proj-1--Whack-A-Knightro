...
public class Mole : MonoBehaviour
{
    ...
    private float hardRate = 0.75f;
    private float bombRate = 0.5f;
    private int lives;
    ...
    private IEnumerator ShowHide(Vector2 start, Vector2 end)
    {
        ...
        // If we miss hitting the mole and its still hittable
        if (hittable)
        {
            hittable = false;
            //for bomb
    ...
    // As the level profgresses it gets harder
    private void SetLevel(int level)
    {
        // As level increases increse the bomb rate (10)
        bombRate = Mathf.Min(level * 0.025f, 1f);

        // Increase the amounts of HardHats 
        hardRate = Mathf.Min(level * 0.5f, .5f);
